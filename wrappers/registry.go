...
func NewRegistry(cc *client.CeloClient) (*RegistryWrapper, error) {
	registry, err := contracts.NewRegistry(params.RegistrySmartContractAddress, cc.Eth)
...
func (w *RegistryWrapper) GetLockedGold(ctx context.Context, block *big.Int) (*contracts.LockedGold, error) {
	addr, err := w.GetAddressForString(ctx, block, LockedGoldRegistryId.String())
	if err != nil {
		return nil, err
	}

	lockedGold, err := contracts.NewLockedGold(addr, w.cc.Eth)
	if err != nil {
		return nil, err
	}
...
func (w *RegistryWrapper) GetAccounts(ctx context.Context, block *big.Int) (*contracts.Accounts, error) {
	addr, err := w.GetAddressForString(ctx, block, AccountsRegistryId.String())
	if err != nil {
		return nil, err
	}

	accounts, err := contracts.NewAccounts(addr, w.cc.Eth)
	if err != nil {
		return nil, err
	}
...
func (w *RegistryWrapper) GetElection(ctx context.Context, block *big.Int) (*contracts.Election, error) {
	addr, err := w.GetAddressForString(ctx, block, ElectionRegistryId.String())
	if err != nil {
		return nil, err
	}

	election, err := contracts.NewElection(addr, w.cc.Eth)
	if err != nil {
		return nil, err
	}
